...
/*
 * M3: slice handed to tasks in FIFO cgroups. A long slice approximates the
 * design's "run to block" make -j behaviour — a CPU-bound task keeps its CPU
 * until it blocks/exits, so only ~nr_cpus are active and context switches (and
 * cache thrashing) drop. 0 means use SCX_SLICE_DFL. Top-level cgroup fairness
 * is still bounded by cgrp_slice_ns. Set via userspace -S US.
 */
const volatile u64 fifo_slice_ns;
...
/*
 * M2: per-cgroup policy override, populated by userspace (-F <cgroup-path>)
 * before attach. Keyed by cgroup id (cgrp->kn->id == cgroup dir inode).
 * A cgroup absent from this map uses the global default (fifo_sched).
 */
struct {
	...
	__type(key, __u64);
	__type(value, __u32);
} cgrp_policy SEC(".maps");

/* Resolve the effective within-cgroup policy for a cgroup context. */
static __always_inline bool cgrp_is_fifo(struct fcg_cgrp_ctx *cgc)
{
	return cgc && cgc->policy == HSCHED_POLICY_FIFO;
}
...
void BPF_STRUCT_OPS(fcg_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (cgrp_is_fifo(cgc)) {
		u64 slice = fifo_slice_ns ? fifo_slice_ns : SCX_SLICE_DFL;
...
void BPF_STRUCT_OPS(fcg_running, struct task_struct *p)
{
	...
	/* M2: vtime accounting is per-cgroup; skip it for FIFO cgroups. */
	cgrp = scx_bpf_task_cgroup(p);
	cgc = find_cgrp_ctx(cgrp);
	if (cgc && !cgrp_is_fifo(cgc)) {
		/*
...
void BPF_STRUCT_OPS(fcg_stopping, struct task_struct *p, bool runnable)
{
	...
	/* M2: vtime charging is per-cgroup — fetch the cgroup up front. */
	cgrp = scx_bpf_task_cgroup(p);
	cgc = find_cgrp_ctx(cgrp);

	/*
	 * Scale the execution time by the inverse of the weight and charge.
	 * Only for vtime cgroups; FIFO cgroups don't maintain task vtime.
	 *
			     ...
			     struct scx_cgroup_init_args *args)
{
	...
	/*
	 * M2: resolve this cgroup's within-cgroup policy. An explicit per-cgroup
	 * override (cgrp_policy map, set by userspace) wins; otherwise fall back
	 * to the global default selected with -f (fifo_sched).
	 */
	{
		u32 *ovr = bpf_map_lookup_elem(&cgrp_policy, &cgid);

		if (ovr)
			cgc->policy = *ovr;
		else
			cgc->policy = fifo_sched ? HSCHED_POLICY_FIFO
						 : HSCHED_POLICY_VTIME;
	}
